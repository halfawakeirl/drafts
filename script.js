...
async function runDictionaryTyping() {
  ...
  const defsEl = document.getElementById("dictDefs");
  if (!wordEl || !pronEl || !posEl || !defsEl) return; // only runs on pages that have it

  // Edit these to match your vibe
  const entry = {
    word: "draft",
    pron: "/dræft/",
    pos: "noun",
    defs: [
      "a message written but never sent.",
      "the thoughts that linger after silence.",
      "a private version of the truth, kept off-screen."
    ]
  };

  await typewriter(wordEl, entry.word, 90);
  await sleep(200);
  await typewriter(pronEl, entry.pron, 35);
  await sleep(120);
  await typewriter(posEl, entry.pos, 40);
  await sleep(150);
  ...
  for (const line of entry.defs) {
    ...
    await typewriter(li, line, 22);
    await sleep(120);
  }
}

// Run after load
document.addEventListener("DOMContentLoaded", runDictionaryTyping);
document.addEventListener("DOMContentLoaded", () => {
